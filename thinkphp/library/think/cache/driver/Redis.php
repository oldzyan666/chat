...
class Redis extends Driver
{
     ...
     * @return bool
     */
    public  function hSet($name, $keys, $value)
    {
        if ($this->handler->hExists($name,$keys)){
            $oVal=$this->handler->hGet($name,$keys);
            $this->handler->hSet($name, $keys, $value);
            $nVal=$this->handler->hGet($name,$keys);
            return $oVal==$nVal ? 0:1;
        }else{
            return $this->handler->hSet($name, $keys, $value);
        }
    }
    /**
     * hash获取键值
     ...
     * @return string
     */
    public  function hGet($name, $keys)
    {
        return $this->handler->hGet($name, $keys);
    }
    /**
     * hash获取长度
     * @access public
    ...
    {
        return $this->handler->hMset($name,$data);
    }
    /**
     * 批量获取数据
     * @access public
     ...
     * @return bool｜array
     */
    public  function hMGet($name,$data)
    {
        return $this->handler->hMGet($name,$data);
    }
    /**
     * 删除hash
     * @access public
